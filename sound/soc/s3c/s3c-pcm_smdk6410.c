...
static void s3c24xx_pcm_enqueue(struct snd_pcm_substream *substream)
{
	...
	while (prtd->dma_loaded < prtd->dma_limit) {
		...
		if (ret == 0) {
			...
			printk("%s: fail to loaded dma work!\n", __func__);
			break;
		}
	}
				...
				enum s3c2410_dma_buffresult result)
{
	...
	if (result == S3C2410_RES_ABORT || result == S3C2410_RES_ERR){
		...
		if (FLASH_PROBLEM_PATCH == 0)
			prtd->dma_loaded--;  

			s3c24xx_pcm_enqueue(substream);
		}

		if (FLASH_PROBLEM_PATCH == 1)
		prtd->dma_loaded--;
...
static int __init s3s_soc_platform_init(void)
{
	return snd_soc_register_platform(&s3c24xx_soc_platform);
}
module_init(s3s_soc_platform_init);

static void __exit s3s_soc_platform_exit(void)
{
	snd_soc_unregister_platform(&s3c24xx_soc_platform);
}
module_exit(s3s_soc_platform_exit);
