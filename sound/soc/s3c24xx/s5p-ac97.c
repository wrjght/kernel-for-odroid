/*
 * s3c-ac97.c  --  ALSA Soc Audio Layer
 *
...
//#define CONFIG_SOUND_WM9713_INPUT_STREAM_MIC 1
	...
	unsigned short val)
{
	...
	stat = readl(s3c_ac97.regs + S3C_AC97_CODEC_CMD);
	data = (stat & 0xffff);
...
static int s3c_ac97_probe(struct platform_device *pdev, struct snd_soc_dai *dai)
{
	int ret;

	s3cdbg("Entered %s\n", __FUNCTION__);

	s3c_ac97.regs = ioremap(S3C_PA_AC97, 0x100);
	if (s3c_ac97.regs == NULL)
		return -ENXIO;

	s3c_ac97.ac97_clk = clk_get(&pdev->dev, "ac97");
	if (s3c_ac97.ac97_clk == NULL) {
		printk(KERN_ERR "s3c-ac97 failed to get ac97_clock\n");
		iounmap(s3c_ac97.regs);
		return -ENODEV;
	}
	clk_enable(s3c_ac97.ac97_clk);

	ret = request_irq(IRQ_AC97, s3c_ac97_irq,
...
static void s3c_ac97_remove(struct platform_device *pdev, struct snd_soc_dai *dai)
{
	s3cdbg("Entered %s\n", __FUNCTION__);
					  ...
					  struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		rtd->dai->cpu_dai->dma_data = &s3c_ac97_pcm_stereo_out;
	else
#ifdef CONFIG_SOUND_WM9713_INPUT_STREAM_MIC
		rtd->dai->cpu_dai->dma_data = &s3c_ac97_mic_mono_in;
#else /* Input Stream is LINE-IN */
		rtd->dai->cpu_dai->dma_data = &s3c_ac97_pcm_stereo_in;
#endif
						...
						struct snd_soc_dai *dai)
{
	...
	s3c_ac97_write(0,0x02, 0x0);	
	s3c_ac97_write(0,0x04, 0x0);	
	s3c_ac97_write(0,0x05, 0x0);	
	...
	struct snd_pcm_hw_params *params)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		...
		cpu_dai->dma_data = &s3c_ac97_mic_mono_in;
...
MODULE_DESCRIPTION("AC97 driver for the Samsung s3c chip");
MODULE_LICENSE("GPL");
