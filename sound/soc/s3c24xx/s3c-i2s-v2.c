...
#define dump_i2s()	do{ \
				printk("%s:%d Base=%x\n", __func__, __LINE__, i2s->dma_playback->dma_addr); \
				printk("\tS3C2412_IISCON=0x%x\tS3C2412_IISMOD=0x%x\n", \
					readl(regs + S3C2412_IISCON), readl(regs + S3C2412_IISMOD)); \
				printk("\tS3C2412_IISPSR=0x%x\tS3C2412_IISFIC=0x%x\n", \
					readl(regs + S3C2412_IISPSR), readl(regs + S3C2412_IISFIC)); \
				printk("\tS5P_CLK_SRC0=0x%x\tS5P_CLK_SRC3=0x%x\n", \
					readl(S5P_CLK_SRC0), readl(S5P_CLK_SRC3)); \
				printk("\tS5P_CLK_DIV4=0x%x\tS5P_LPMP_MODE_SEL=0x%x\n", \
					readl(S5P_CLK_DIV4), readl(S5P_LPMP_MODE_SEL)); \
				printk("\tS5P_CLKGATE_D20=0x%x\tS5P_CLKGATE_D15=0x%x\n", \
					readl(S5P_CLKGATE_D20), readl(S5P_CLKGATE_D15)); \
				printk("\tS5P_SCLKGATE1=0x%x\tS5P_NORMAL_CFG=0x%x\n", \
					readl(S5P_SCLKGATE1), readl(S5P_NORMAL_CFG)); \
				printk("\tS5P_BLK_PWR_STAT=0x%x\n", \
...
#if defined(CONFIG_PLAT_S5PC1XX) || defined(CONFIG_PLAT_S5PC11X) \
					|| defined(CONFIG_PLAT_S5P64XX)
