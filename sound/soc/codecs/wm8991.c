...
#if defined (CONFIG_SND_S5PC100_SMBUS_I2C)
	#include <linux/i2c.h>
#else
	...
	unsigned int reg)
{
	...
	BUG_ON(reg > (ARRAY_SIZE(wm8991_reg)) - 1);
	return cache[reg];
}
	...
	unsigned int reg, unsigned int value)
{
	...
	if (reg == 0 || reg > ARRAY_SIZE(wm8991_reg) - 1)
		return;
...
SOC_SINGLE("Speaker Output Attenuation Volume", WM8991_CLASSD4,
...
SOC_DAPM_SINGLE_TLV("LIN2 Volume", WM8991_INPUT_MIXER5, WM8991_LI2BVOL_SHIFT,
	7, 0, in_mix_tlv),
SOC_DAPM_SINGLE("LINPGA12 Switch", WM8991_INPUT_MIXER3, WM8991_L12MNB_BIT, 1, 0),
SOC_DAPM_SINGLE("LINPGA34 Switch", WM8991_INPUT_MIXER3, WM8991_L34MNB_BIT, 1, 0),
};
...
static int wm8991_mute(struct snd_soc_dai *dai, int mute)
{
	...
	val  = wm8991_read_reg_cache(codec, WM8991_DAC_CTRL) & ~WM8991_DAC_MUTE;
	
	if (mute)
		wm8991_write(codec, WM8991_DAC_CTRL, val | WM8991_DAC_MUTE);
	else
		wm8991_write(codec, WM8991_DAC_CTRL, val);
	...
	enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_PREPARE:
		...
		{
			val = wm8991_read_reg_cache(codec, WM8991_POWER_MANAGEMENT_1);
			wm8991_write(codec, WM8991_POWER_MANAGEMENT_1, val|WM8991_SPK_ENA|WM8991_LOUT_ENA|WM8991_ROUT_ENA);

			val = wm8991_read_reg_cache(codec, WM8991_POWER_MANAGEMENT_3);
			wm8991_write(codec, WM8991_POWER_MANAGEMENT_3, val|WM8991_LOMIX_ENA|WM8991_ROMIX_ENA);
			
			val = wm8991_read_reg_cache(codec, WM8991_ANTIPOP2);
			val &= (~WM8991_BUFIOEN);
			wm8991_write(codec, WM8991_ANTIPOP2, val&(~WM8991_BUFIOEN));
		}
		else if(codec->bias_level == SND_SOC_BIAS_ON)
		{
		}
		break;
...
//			wm8991_write(codec, WM8991_ANTIPOP1, WM8991_DIS_LOUT | WM8991_DIS_ROUT);
//
//			/* Delay to allow output caps to discharge */
//			msleep(msecs_to_jiffies(300));
//
//			wm8991_write(codec, WM8991_ANTIPOP2, WM8991_SOFTST |
//				     WM8991_BUFDCOPEN | WM8991_POBCTRL);
//
//			/* Enable POBCTRL, SOFT_ST, VMIDTOG and BUFDCOPEN */
//			val = wm8991_read_reg_cache(codec, WM8991_ANTIPOP2);
//			wm8991_write(codec, WM8991_ANTIPOP2, val&~WM8991_VMIDTOG);
//
//			/* Disable all output discharge bits */
//			val = wm8991_read_reg_cache(codec, WM8991_ANTIPOP2);
//			wm8991_write(codec, WM8991_ANTIPOP1, val&~(WM8991_DIS_LOUT|WM8991_DIS_ROUT));
//
//			/* Enable outputs */
//			wm8991_write(codec, WM8991_POWER_MANAGEMENT_1, 0x00);
		
//			msleep(msecs_to_jiffies(50));
//			wm8991_write(codec, WM8991_LEFT_OUTPUT_VOLUME, 0x6e | (1<<8));
//			wm8991_write(codec, WM8991_RIGHT_OUTPUT_VOLUME, 0x6e | (1<<8));

			wm8991_write(codec, WM8991_LEFT_OUTPUT_VOLUME, 0x6e | (1<<8));
			wm8991_write(codec, WM8991_RIGHT_OUTPUT_VOLUME, 0x6e | (1<<8));
			wm8991_write(codec, WM8991_SPEAKER_ATTN, 0x00);
			wm8991_write(codec, WM8991_CLASSD4, 0x73);
			wm8991_write(codec, WM8991_CLASSD3, 0x1d);

			/* Enable VMID at 2x250k */
//			wm8991_write(codec, WM8991_POWER_MANAGEMENT_1, 0x13);
//
...
//			/* Enable BUFIOEN */
//			val = wm8991_read_reg_cache(codec, WM8991_ANTIPOP2);
//			val &= (WM8991_SOFTST|WM8991_BUFDCOPEN|WM8991_POBCTRL);
//			wm8991_write(codec, WM8991_ANTIPOP2, val);
//
//			/* Disable outputs */
//			wm8991_write(codec, WM8991_POWER_MANAGEMENT_1, 0x13);
//
...
//			wm8991_write(codec, WM8991_ANTIPOP2, WM8991_BUFIOEN);
		...
		{
			/* ON -> standby (PCM Release)*/

			/* Enable VREF, VMID at 2x250k , Disable outputs */
	...
	case SND_SOC_BIAS_OFF:
		...
		/* mute DAC */
		val = wm8991_read_reg_cache(codec, WM8991_DAC_CTRL);
		wm8991_write(codec, WM8991_DAC_CTRL, val | WM8991_DAC_MUTE);

		/* Enable any disabled outputs */
		wm8991_write(codec, WM8991_POWER_MANAGEMENT_1, 0x1f03);

		/* Disable VMID */
		wm8991_write(codec, WM8991_POWER_MANAGEMENT_1, 0x1f01);

		msleep(msecs_to_jiffies(300));

		/* Enable all output discharge bits */
		wm8991_write(codec, WM8991_ANTIPOP1, WM8991_DIS_LLINE |
			WM8991_DIS_RLINE | WM8991_DIS_OUT3 |
			WM8991_DIS_OUT4 | WM8991_DIS_LOUT |
			WM8991_DIS_ROUT);

		/* Disable VREF */
		wm8991_write(codec, WM8991_POWER_MANAGEMENT_1, 0x03);

		wm8991_write(codec, WM8991_POWER_MANAGEMENT_2, 0x00);
		wm8991_write(codec, WM8991_POWER_MANAGEMENT_3, 0x00);

		/* disable POBCTRL, SOFT_ST and BUFDCOPEN */
		wm8991_write(codec, WM8991_ANTIPOP2, 0x0);
		break;
	}
...
static int wm8991_dai_trigger(struct snd_pcm_substream *substream, int cmd, struct snd_soc_dai *dai)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct snd_soc_device *socdev = rtd->socdev;
	struct snd_soc_codec *codec = socdev->codec;
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		if(codec->bias_level == SND_SOC_BIAS_STANDBY)
			wm8991_set_bias_level(codec, SND_SOC_BIAS_PREPARE);
		break;
...
static void wm8991_set_micin(struct snd_soc_codec *codec)
{
	u16 reg;

	// need for Capture block Power Enable
	reg = wm8991_read_reg_cache(codec, WM8991_POWER_MANAGEMENT_1);
	reg |= WM8991_MICBIAS_ENA;
	wm8991_write(codec, WM8991_POWER_MANAGEMENT_1, reg);

	reg = wm8991_read_reg_cache(codec, WM8991_POWER_MANAGEMENT_2);
	reg |= WM8991_LIN12_ENA;
	reg |= WM8991_AINL_ENA;
	wm8991_write(codec, WM8991_POWER_MANAGEMENT_2, reg);

	// Input Mixer Path/Gain Setting
	reg = wm8991_read_reg_cache(codec, WM8991_AUDIO_INTERFACE_4);
	reg |= WM8991_ALRCGPIO1;
	wm8991_write(codec, WM8991_AUDIO_INTERFACE_4, reg);

	reg = wm8991_read_reg_cache(codec, WM8991_AUDIO_INTERFACE_1);
	reg &= ~(WM8991_AIFADCL_SRC|WM8991_AIFADCR_SRC);
	reg |= WM8991_AIF_WL_16BITS;
	reg |= WM8991_AIF_TMF_I2S;
	wm8991_write(codec, WM8991_AUDIO_INTERFACE_1, reg);
	
	reg = wm8991_read_reg_cache(codec, WM8991_LEFT_LINE_INPUT_1_2_VOLUME);
	reg = (WM8991_LIN12VOL_MASK<<WM8991_LIN12VOL_SHIFT);
	reg &= ~WM8991_LI12MUTE;
	reg |= WM8991_IPVU;
	wm8991_write(codec, WM8991_LEFT_LINE_INPUT_1_2_VOLUME, reg);
	
	reg = wm8991_read_reg_cache(codec, WM8991_INPUT_MIXER2);
	reg |= WM8991_LMN1;
	reg |= WM8991_LMP2;
	wm8991_write(codec, WM8991_INPUT_MIXER2, reg);

	reg = wm8991_read_reg_cache(codec, WM8991_INPUT_MIXER3);
	reg |= WM8991_L12MNB;
	reg |= (WM8991_LDBVOL_MASK<<WM8991_LDBVOL_SHIFT);
	wm8991_write(codec, WM8991_INPUT_MIXER3, reg);

	reg = 0x0000;
	wm8991_write(codec, WM8991_INPUT_MIXER1, reg);

	reg = wm8991_read_reg_cache(codec, WM8991_ADC_CTRL);
	reg |= WM8991_ADC_HPF_ENA;
	reg |= (WM8991_ADC_HPF_CUT_MASK << WM8991_ADC_HPF_CUT_SHIFT);
	wm8991_write(codec, WM8991_ADC_CTRL, reg);

	reg = wm8991_read_reg_cache(codec, WM8991_LEFT_ADC_DIGITAL_VOLUME);
	reg = (0xd0&WM8991_ADCL_VOL_MASK) << WM8991_ADCL_VOL_SHIFT;
	reg |= WM8991_ADC_VU;
	wm8991_write(codec, WM8991_LEFT_ADC_DIGITAL_VOLUME, reg);
...
static void wm8991_delayed_work(struct work_struct *work)
{
...
//	struct snd_soc_codec *codec =
//		container_of(work, struct snd_soc_codec, delayed_work.work);
//	wm8991_set_bias_level(codec, codec->bias_level);
}
...
static int wm8991_add_i2c_device(struct platform_device *pdev,
				 const struct wm8991_setup_data *setup)
{
	...
	strlcpy(info.type, "wm8991", I2C_NAME_SIZE);
...
static int wm8991_probe(struct platform_device *pdev)
{
	...
	info("WM8991 Audio Codec %s", WM8991_VERSION);
	...
	INIT_DELAYED_WORK(&codec->delayed_work, wm8991_delayed_work);

//#if defined (CONFIG_I2C) || defined (CONFIG_I2C_MODULE)
#if defined (CONFIG_SND_S5PC100_SMBUS_I2C)
	if (setup->i2c_address) {
		codec->hw_write = (hw_write_t)i2c_master_send;
		ret = wm8991_add_i2c_device(pdev, setup);
	}
#else
	/* s5pc100 gpio i2c control */
	wm8991_i2c_gpio_init();
	codec->hw_write = (hw_write_t)wm8991_hw_write;
	codec->hw_read = (hw_read_t)wm8991_hw_read;
...
/* out path  */
int wm8991_set_outpath(u8 path)
{
	struct snd_soc_device *socdev = wm8991_socdev;
	struct snd_soc_codec *codec = socdev->codec;
	u16 mixer1, mixer2, spkmixer;

	mixer1 = wm8991_read_reg_cache(codec, WM8991_OUTPUT_MIXER1);
	mixer2 = wm8991_read_reg_cache(codec, WM8991_OUTPUT_MIXER2);
	spkmixer = wm8991_read_reg_cache(codec, WM8991_SPEAKER_MIXER);

	mixer1 = 0x0000;
	mixer2 = 0x0000;
	spkmixer = 0x0000;
//	spkmixer &= ~(WM8991_LB2SPK|WM8991_RB2SPK|WM8991_LOPGASPK|WM8991_ROPGASPK);
	...
	switch(path)
	{
		case WM8991_NONE :
			mixer1 &= ~WM8991_LDLO;
			mixer2 &= ~WM8991_RDRO;
			spkmixer &= ~(WM8991_LDSPK|WM8991_RDSPK);
			break;
		case WM8991_SPK :
			mixer1 &= ~WM8991_LDLO;
			mixer2 &= ~WM8991_RDRO;
	...
	wm8991_write(codec, WM8991_OUTPUT_MIXER1, mixer1);
	wm8991_write(codec, WM8991_OUTPUT_MIXER2, mixer2);
	wm8991_write(codec, WM8991_SPEAKER_MIXER, spkmixer);
