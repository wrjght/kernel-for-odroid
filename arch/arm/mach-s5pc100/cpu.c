...
extern __init void s3c_pwmclk_init(void);
...
*/
	...
	tmp &= (S5P_CFG_WFI_CLEAN | 0x1 <<31) ; //CFG_DEEP_IDLE mask
//	tmp &= S5P_CFG_WFI_CLEAN;

	tmp |= (S5P_CFG_WFI_IDLE); //0x0 Enter to IDLE Mode
	__raw_writel(tmp, S5P_PWR_CFG);

	/* Set Syscon Interrupt */
	tmp = __raw_readl(S5P_OTHERS);
	tmp |= (1 << S5P_OTHER_SYS_INT); //occured system_reset due to ARM off
