...
void s3c6410_pm_do_save_phy(struct sleep_save_phy *ptr, int count)
{
	void __iomem *target_reg;
	target_reg = ioremap(ptr->reg , 4);

	for (; count > 0; count--, ptr++) {
		ptr->val = readl(target_reg);
	}
}
...
void s3c6410_pm_do_restore_phy(struct sleep_save_phy *ptr, int count)
{
	void __iomem *target_reg;
	target_reg = ioremap(ptr->reg , 4);

	for (; count > 0; count--, ptr++) {
		writel(ptr->val, target_reg);
	}
}
