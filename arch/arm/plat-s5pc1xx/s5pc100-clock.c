...
#define CLK_DIV_CHANGE_BY_STEP 1
#else
...
/*	u32 size;
	if(S5PC1XX_FREQ_TAB)
		size = ARRAY_SIZE(s5p_clk_tab_evt1);
	else
		size = ARRAY_SIZE(s3c_cpu_clk_tab_533MHz);
*/
	return ARRAY_SIZE(s5p_dout_clk_tab);
}
...
#endif
int s5pc1xx_clk_set_rate(struct clk *clk, unsigned int target_freq,
                                unsigned int index )
{
	int cur_freq;
	unsigned int mask;
	u32 val;
	u32 size;
	static const u32 (*cpu_clk_tab)[8];

	cpu_clk_tab = s5p_dout_clk_tab;
	...
	{
		printk("=DVFS ERR target_freq (%d) != cpu_tab_freq (%d)\n", index, target_freq);
		return 0;
	}

	cur_freq = (int)s5pc1xx_clk_doutarm_get_rate(clk);

	/* rounding cpu frequency */
	cur_freq = s5pc1xx_doutarm_roundrate(clk,cur_freq);
	...
	if(cur_freq == cpu_clk_tab[index][0])
	{
		...
		// change clock dividor
#if CLK_DIV_CHANGE_BY_STEP
	if (dvfs_change_direction == 1) {
		mask = (~(S5P_CLKDIV0_PCLKD0_MASK));
		val = __raw_readl(S5P_CLK_DIV0) & mask;
		val |= (cpu_clk_tab[index][5]) << S5P_CLKDIV0_PCLKD0_SHIFT;
		__raw_writel(val, S5P_CLK_DIV0);

		mask = (~(S5P_CLKDIV0_D0_MASK));
		val = __raw_readl(S5P_CLK_DIV0) & mask; // can be removed
		val |= (cpu_clk_tab[index][4]) << S5P_CLKDIV0_D0_SHIFT;
		__raw_writel(val, S5P_CLK_DIV0);

		mask = (~(S5P_CLKDIV0_ARM_MASK));
		val = __raw_readl(S5P_CLK_DIV0) & mask;// can be removed
		val |= (cpu_clk_tab[index][3]) << S5P_CLKDIV0_ARM_SHIFT;
		__raw_writel(val, S5P_CLK_DIV0);

	} else if (dvfs_change_direction == 0) {
		mask = (~(S5P_CLKDIV0_ARM_MASK));
		val = __raw_readl(S5P_CLK_DIV0) & mask;
		val |= (cpu_clk_tab[index][3]) << S5P_CLKDIV0_ARM_SHIFT;
		__raw_writel(val, S5P_CLK_DIV0);
		...
		mask = (~(S5P_CLKDIV0_ARM_MASK)) & (~(S5P_CLKDIV0_D0_MASK)) & (~(S5P_CLKDIV0_PCLKD0_MASK));
		val = __raw_readl(S5P_CLK_DIV0) & mask;
		val |= (cpu_clk_tab[index][3]) << S5P_CLKDIV0_ARM_SHIFT;
		val |= (cpu_clk_tab[index][4]) << S5P_CLKDIV0_D0_SHIFT;
		val |= (cpu_clk_tab[index][5]) << S5P_CLKDIV0_PCLKD0_SHIFT;
...
int s5pc1xx_clk_doutarm_set_rate(struct clk *clk, unsigned long rate)
{
	...
	ret = s5pc1xx_clk_set_rate(clk, rate, index);
	return 0;
#else	
	struct clk *temp_clk = clk;
...
int s5pc1xx_clk_D1sync_change(int index) {
	unsigned int div_hclk,div_pclk,val, mask;
	static const u32 (*cpu_clk_tab)[8];
	u32 size;
	...
	val = __raw_readl(S5P_CLK_DIV1);

	div_hclk = (val & S5P_CLKDIV1_D1_MASK) >> S5P_CLKDIV1_D1_SHIFT;
	div_pclk = (val & S5P_CLKDIV1_PCLKD1_MASK) >> S5P_CLKDIV1_PCLKD1_SHIFT;
	if ((div_hclk == cpu_clk_tab[index][6]) && (div_pclk == cpu_clk_tab[index][7])) {
		return 0;
	} else {
#if CLK_DIV_CHANGE_BY_STEP
		if (dvfs_change_direction ==1) {

			mask = (~(S5P_CLKDIV1_PCLKD1_MASK));
			val = val & mask;
			val |= (cpu_clk_tab[index][7]) << S5P_CLKDIV1_PCLKD1_SHIFT;
			__raw_writel(val, S5P_CLK_DIV1);

			val = __raw_readl(S5P_CLK_DIV1);// can be removed
			mask = (~(S5P_CLKDIV1_D1_MASK));
			val = val & mask;
			val |= (cpu_clk_tab[index][6]) <<S5P_CLKDIV1_D1_SHIFT;
			__raw_writel(val, S5P_CLK_DIV1);
			...
			mask = (~(S5P_CLKDIV1_D1_MASK)) & (~(S5P_CLKDIV1_PCLKD1_MASK));
			val = val & mask;
			val |= (cpu_clk_tab[index][6]) << S5P_CLKDIV1_D1_SHIFT;
			val |= (cpu_clk_tab[index][7]) << S5P_CLKDIV1_PCLKD1_SHIFT;
			__raw_writel(val, S5P_CLK_DIV1);
