				...
				int plane, int size, int align)
{
	...
	for (i = 0; i < cap->nr_bufs; i++) {
		/* buff start address align */ 
		ctrl->mem.curr = PAGE_ALIGN(ctrl->mem.curr);
...
static int fimc_alloc_buffers_for_yuv422(struct fimc_control *ctrl, 
				int size, int align)
{
	struct fimc_capinfo *cap = ctrl->cap;
	int i, addr;
	...
	for (i = 0; i < cap->nr_bufs; i++) {		
		...
		cap->bufs[i].length[FIMC_ADDR_Y] = size;
		cap->bufs[i].length[FIMC_ADDR_CB] = size/2;
		cap->bufs[i].length[FIMC_ADDR_CR] = size/2;

		for (addr = 0; addr < 3; addr++) {
			fimc_dma_alloc(ctrl, &cap->bufs[i], addr, align);
			if (!cap->bufs[i].base[addr])
				goto err_alloc;
		}
...
err_alloc:
	for (i = 0; i < cap->nr_bufs; i++) {		
		for (addr = 0; addr < 3; addr++) {
			if (cap->bufs[i].base[addr])
				fimc_dma_free(ctrl, &cap->bufs[i], addr);
		}
		memset(&cap->bufs[i], 0, sizeof(cap->bufs[i]));
	}
...
static int fimc_alloc_buffers_for_yuv420(struct fimc_control *ctrl, 
				int size, int align)
{
	struct fimc_capinfo *cap = ctrl->cap;
	int i, addr;
	...
	for (i = 0; i < cap->nr_bufs; i++) {
		...
		cap->bufs[i].length[FIMC_ADDR_Y] = size;
		cap->bufs[i].length[FIMC_ADDR_CB] = size/4;
		cap->bufs[i].length[FIMC_ADDR_CR] = size/4;
...
int fimc_reqbufs_capture(void *fh, struct v4l2_requestbuffers *b)
{
	...
	case V4L2_PIX_FMT_YUV422P:	/* fall through */
		ret = fimc_alloc_buffers_for_yuv422(ctrl, cap->fmt.width * cap->fmt.height, 0);
		break;
	...
	case V4L2_PIX_FMT_NV12T:		
		ret = fimc_alloc_buffers_for_yuv420(ctrl, cap->fmt.width * cap->fmt.height, 0);
		break;
...
int fimc_s_ctrl_capture(void *fh, struct v4l2_control *c)
{
	...
	struct fimc_global *fimc = get_fimc_dev();	
	int ret = 0;
	struct fimc_s3c_effect *effect = kmalloc(sizeof(*effect), GFP_KERNEL);;	

	dev_dbg(ctrl->dev, "%s\n", __func__);

	mutex_lock(&ctrl->v4l2_lock);

	effect->type = EFFECT_ORIGINAL;
	effect->pat_cb = 0;
	effect->pat_cr = 0;
	...
	case V4L2_CID_EFFECT_ORIGINAL:
		effect->type = EFFECT_ORIGINAL;
		fimc_hwset_change_effect(ctrl, effect);
		break;
		
	case V4L2_CID_EFFECT_NEGATIVE:
		effect->type = EFFECT_NEGATIVE;
		fimc_hwset_change_effect(ctrl, effect);
		break;
	
	case V4L2_CID_EFFECT_EMBOSSING:
		effect->type = EFFECT_EMBOSSING;
		fimc_hwset_change_effect(ctrl, effect);
		break;
	
	case V4L2_CID_EFFECT_ARTFREEZE:
		effect->type = EFFECT_ARTFREEZE;
		fimc_hwset_change_effect(ctrl, effect);
		break;
	
	case V4L2_CID_EFFECT_SILHOUETTE:
		effect->type = EFFECT_SILHOUETTE;
		fimc_hwset_change_effect(ctrl, effect);
		break;
	
	case V4L2_CID_EFFECT_ARBITRARY:
		effect->type = EFFECT_ARBITRARY;
		effect->pat_cb = PAT_CB(c->value);
		effect->pat_cr = PAT_CR(c->value);
		fimc_hwset_change_effect(ctrl, effect);
		break;
						
	case V4L2_CID_TEST_PATTERN:
		ctrl->cam = fimc->camera[FIMC_TPID];
		fimc_hwset_test_pattern(ctrl, c->value);
		break;
...
int fimc_streamon_capture(void *fh)
{
	...
	fimc_hwset_output_yuv(ctrl, &cap->fmt);
