...
int isMFCRunning = 0;
int mIsCalledDvfsLock  	= 0;	// 0 : Do not use DVFS lock , 1: Used DVFS lock
...
//	writel(readl(S5P_MTC_STABLE)|(0<<1),S5P_MTC_STABLE);
	...
	mfc_debug("Power Status [1]:%x\n",readl(S5P_BLK_PWR_STAT));
		...
		if(mIsCalledDvfsLock == 1)		
			s5pc100_unlock_dvfs_high_level(DVFS_LOCK_TOKEN_1);
#endif
...
static int s3c_mfc_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_MFC_DEC_INIT:
		...
		mIsCalledDvfsLock = 0;
#ifdef CONFIG_CPU_FREQ
		if(frame_size > 1380000)
		{
			mIsCalledDvfsLock = 1;		
			s5pc100_lock_dvfs_high_level(DVFS_LOCK_TOKEN_1);
		}		
#endif
		memset(&local_param, 0, sizeof(local_param));
