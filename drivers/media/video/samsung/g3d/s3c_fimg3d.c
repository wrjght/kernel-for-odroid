...
//#define CONFIG_PM_PWR_GATING


//#include <asm/plat-s3c24xx/dma.h>
#include <asm/cacheflush.h>	// sy82.yoon(2009.04.21)
...
#ifndef G3D_CHUNK
#define G3D_CHUNK_NUM 0
#else
#define G3D_CHUNK_NUM 16		// 32MB 
#endif

//#define G3D_CHUNK_SIZE SZ_4M
#define G3D_CHUNK_SIZE SZ_2M
#define G3D_RESERVED_MEM_SIZE (G3D_CHUNK_SIZE * G3D_CHUNK_NUM)
#define G3D_SAFE_CHUNK_NUM		G3D_CHUNK_NUM

// 2009.09.03 modified by hk 
...
s3c_g3d_bootmem g3d_bootm[G3D_CHUNK_NUM];


void g3d_bootm_dump(void)
{
	
	int loop_i;
	DEBUG("<g3d_bootm_dump>\n");
	for(loop_i = 0; loop_i < G3D_CHUNK_NUM; loop_i++ ){
		DEBUG("g3d_bootm[%d]: fd=0x%x in_use=%d\n",loop_i,g3d_bootm[loop_i].file_desc_id,g3d_bootm[loop_i].in_used);
	}
}
...
//	DEBUG("3d dma transfer completed.\n");
...
int s3c_g3d_open(struct inode *inode, struct file *file)
{
    ...
    DEBUG("[3D] s3c_g3d_open is called -------------------------------------------- \n");
...
int s3c_g3d_release(struct inode *inode, struct file *file)
{
    ...
    DEBUG("[3D] s3c_g3d_release is called -------------------------------------------- \n");
...
static int s3c_g3d_ioctl(struct inode *inode, struct file *file,unsigned int cmd, unsigned long arg)
{
	...
	case WAIT_FOR_FLUSH:
		...
		//DEBUG("wait for flush (arg=0x%lx)\n",arg);


		timer = 1000000;

		while(timer) {
			wait_event_interruptible_timeout(waitq, (interrupt_already_recevied>0), 1*HZ);			__raw_writel(0,s3c_g3d_base+FGGB_INTMASK);

			__raw_writel(0,s3c_g3d_base+FGGB_INTMASK);
			interrupt_already_recevied = 0;
			//if(interrupt_already_recevied==0)interruptible_sleep_on(&waitq);
			val = __raw_readl(s3c_g3d_base+FGGB_PIPESTATE);
			//DEBUG("in while read pipestate = 0x%x\n",val);
			if(val & arg){
			} else{
				break;
			}
	...
	//	DEBUG("wait for end of dma operation\n");
		wait_for_completion(&complete);
	//	DEBUG("dma operation is performed\n");
...
//		g3d_bootm_dump();
	...
	case S3C_3D_MEM_SHARE_FREE:	
		...
		if(copy_from_user(&param, (struct s3c_3d_mem_alloc *)arg, sizeof(struct s3c_3d_mem_alloc))){
			printk("ERR: Cache Invalid Error\n");
	...
	case S3C_3D_CRITICAL_SECTION:
                ...
                {/*kick power off*/
                                ...
                                g_G3D_SelfPowerOFF=True;
                                DEBUG("[3D] Power off - no wait\n");
                                */
                                
                }
                else if(g_G3D_CriticalFlag>0)
                {/*kick power on*/
                            if(s5p_domain_off_check(S5PC100_POWER_DOMAIN_G3D))                  
                            {/*if powered off*/                        
                                            if( g_G3D_SelfPowerOFF)
                                            {/*powered off by 3D PM or by Resume*/
                                                /*power on*/
                				 s5p_power_gating(S5PC100_POWER_DOMAIN_G3D, DOMAIN_ACTIVE_MODE);
                				 if(s5p_wait_blk_pwr_ready(S5PC100_POWER_DOMAIN_G3D)){
                					printk("[3D] s3c_wait_blk_pwr_ready err\n");
                					mutex_unlock(&pm_status_lock);
                					return -EFAULT;	
                				 }                                                                   
                                                clk_g3d_enable();
                                                /*Need here??*/
        ...
        //      DEBUG("S3C_3D_CRITICAL_SECTION: param_pm.criticalSection=%d\n",param_pm.criticalSection);
                mutex_unlock(&pm_critical_section_lock);
                break;
...
int s3c_g3d_mmap(struct file* filp, struct vm_area_struct *vma)
{
	...
	case MEM_ALLOC :
		...
		//DEBUG("MMAP_ALLOC : virt addr = 0x%p, phys addr = 0x%p, size = %d\n", virt_addr, phys_addr, size);
	...
	default :
		...
		printk("MMAP : vma->end = 0x%p, vma->start = 0x%p, size = %d\n", vma->vm_end, vma->vm_start, size);
...
int s3c_g3d_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_PM_PWR_GATING
	g3d_clock = clk_get(&pdev->dev, "g3d");
	if(g3d_clock == NULL) {
		printk(KERN_ERR PFX "failed to find g3d clock source\n");
		return -ENOENT;
	}
	...
	for( loop_i = 0; loop_i < G3D_CHUNK_NUM; loop_i++ ){
		g3d_bootm[loop_i].vir_addr = kmalloc(G3D_CHUNK_SIZE, GFP_KERNEL);
		g_uiFreeMemMap |=(0x1 << loop_i);
		...
		DEBUG("%d th virt_addr = 0x%p, phy_addr = 0x%p\n", loop_i, (void *)g3d_bootm[loop_i].vir_addr, (void *)g3d_bootm[loop_i].phy_addr);
	}
// 2009.09.15 added by hk for power gating
#ifdef CONFIG_PM_PWR_GATING
        clk_g3d_disable();
        s5p_power_gating(S5PC100_POWER_DOMAIN_G3D, DOMAIN_LP_MODE);
#endif
...
#ifndef CONFIG_HAS_WAKELOCK
#ifdef CONFIG_ANDROID_POWER
static struct android_early_suspend early_suspend_lp_g3d = {
	.level = ANDROID_EARLY_SUSPEND_LEVEL_BLANK_SCREEN,
	.suspend = s3c_g3d_suspend,
	.resume = s3c_g3d_resume,
};
#endif
#else
static struct early_suspend early_suspend_lp_g3d = {
...
int __init  s3c_g3d_init(void)
{
...
#ifndef CONFIG_HAS_WAKELOCK
#ifdef CONFIG_ANDROID_POWER
	android_register_early_suspend(&early_suspend_lp_g3d);
#endif
#else
	register_early_suspend(&early_suspend_lp_g3d);
#endif
	...
	printk("g3d is registered successfully!");
...
#ifndef CONFIG_HAS_WAKELOCK
#ifdef CONFIG_ANDROID_POWER
	android_unregister_early_suspend(&early_suspend_lp_g3d);
#endif
#else
	unregister_early_suspend(&early_suspend_lp_g3d);
#endif
#endif //CONFIG_LPAUDIO
#endif

	for( loop_i = 0; loop_i < G3D_CHUNK_NUM; loop_i++ ){
		kfree((void *)g3d_bootm[loop_i].vir_addr);
		g_uiFreeMemMap &=~(0x1 << loop_i);							
