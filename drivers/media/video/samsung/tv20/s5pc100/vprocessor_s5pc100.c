...
#ifdef S5P_VP_DEBUG
#define VPPRINTK(fmt, args...)	printk("\t\t[VP] %s: " fmt, __FUNCTION__ , ## args)
#else
					...
					signed char ch3)
{
	...
	if (poly_coeff > VPROC_POLY4_C1_HH || poly_coeff < VPROC_POLY8_Y0_LL ||
		(poly_coeff > VPROC_POLY8_Y3_HH && poly_coeff < VPROC_POLY4_Y0_LL)) {
...
{
	s5p_tv_vp_filter_h_pp e_h_filter;
	s5p_tv_vp_filter_v_pp e_v_filter;
	u8* poly_flt_coeff;
	int i, j;
	...
	*/
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			__s5p_vp_set_poly_filter_coef(VPROC_POLY8_Y0_LL + (i*4) + j,
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < 4; j++) {
			__s5p_vp_set_poly_filter_coef(VPROC_POLY4_C0_LL + (i*4) + j,
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			__s5p_vp_set_poly_filter_coef(VPROC_POLY4_Y0_LL + (i*4) + j,
						*(poly_flt_coeff + 4*j*4 + (3 - i)),
						*(poly_flt_coeff + (4*j + 1)*4 + (3 - i)),
						*(poly_flt_coeff + (4*j + 2)*4 + (3 - i)),
						*(poly_flt_coeff + (4*j + 3)*4 + (3 - i)));
...
s5p_tv_vp_err __s5p_vp_set_brightness_contrast_control(s5p_vp_line_eq eq_num,
						       u32 intc,
						       u32 slope)
{
	VPPRINTK("%d,%d,%d\n\r", eq_num, intc, slope);
...
void __s5p_vp_set_brightness(bool brightness)
{
	...
	g_vp_contrast_brightness = VP_LINE_INTC_CLEAR(g_vp_contrast_brightness) |
...
void __s5p_vp_set_contrast(u8 contrast)
{
	...
	g_vp_contrast_brightness = VP_LINE_SLOPE_CLEAR(g_vp_contrast_brightness) |
...
/*
* initialization - iniization functions are only called under stopping video processor
*/
...
s5p_tv_vp_err __s5p_vp_init_layer(u32 top_y_addr,
				...
				bool ipc_2d)
{
	s5p_tv_vp_err error = VPROC_NO_ERROR;
	...
	error = __s5p_vp_set_bottom_field_address(bottom_y_addr, bottom_c_addr);
...
s5p_tv_vp_err __s5p_vp_init_layer_def_poly_filter_coef(u32 top_y_addr,
						u32 top_c_addr,
						u32 bottom_y_addr,
						u32 bottom_c_addr,
						s5p_endian_type src_img_endian,
...
s5p_tv_vp_err __s5p_vp_init_poly_filter_coef(s5p_vp_poly_coeff poly_coeff,
				signed char ch0,
				signed char ch1,
				signed char ch2,
				signed char ch3)
...
s5p_tv_vp_err __s5p_vp_init_csc_coef(s5p_vp_csc_coeff csc_coeff, u32 coeff)
{
	...
	if (csc_coeff > VPROC_CSC_CR2CR_COEF || csc_coeff < VPROC_CSC_Y2Y_COEF) {
