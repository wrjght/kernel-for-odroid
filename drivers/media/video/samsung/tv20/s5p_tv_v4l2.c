...
#define V4L2PRINTK(fmt, args...)	printk("[V4L2_IF] %s: " fmt, __FUNCTION__ , ## args)
#else
...
u8 hdcp_protocol_status=0; // 0 - hdcp stopped, 1 - hdcp started, 2 - hdcp reset
...
static int s5p_tv_v4l2_querycap(struct file *file, void *fh, struct v4l2_capability *cap)
{
...
/* Priority handling */
static int s5p_tv_v4l2_g_priority(struct file *file, void *fh, enum v4l2_priority *p)
{
	return 0;
}

static int s5p_tv_v4l2_s_priority(struct file *file, void *fh, enum v4l2_priority p)
{
	return 0;
}

/* VIDIOC_ENUM_FMT handlers */
static int s5p_tv_v4l2_enum_fmt_vid_out(struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
	int index = f->index;
...
static int s5p_tv_v4l2_enum_fmt_vid_out_overlay(struct file *file, void *fh, struct v4l2_fmtdesc *f)
{
	int index = f->index;
	...
	memcpy(f, &s5p_tv_o_overlay_fmt_desc[index], sizeof(struct v4l2_fmtdesc));
...
static int s5p_tv_v4l2_g_fmt_vid_out(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT: {
		...
		memcpy(vid_out_fmt->fmt.raw_data, &vparam, sizeof(struct v4l2_pix_format_s5p_tvout));
		break;
	}
...
static int s5p_tv_v4l2_g_fmt_vid_out_overlay(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT_OVERLAY: {
		...
		V4L2PRINTK("[type 0x%08x] : left: [%d], top [%d], width[%d], height[%d]\n",
			   ...
			   s5ptv_status.vl_basic_param.src_width,
			   s5ptv_status.vl_basic_param.src_height);
		memcpy(vid_out_fmt->fmt.raw_data, &vparam, sizeof(struct v4l2_window_s5p_tvout));
		break;
	}
...
static int s5p_tv_v4l2_s_fmt_vid_out_overlay(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT_OVERLAY: {
		...
		struct v4l2_window_s5p_tvout vparam;
		memcpy(&vparam, vid_out_fmt->fmt.raw_data, sizeof(struct v4l2_window_s5p_tvout));

		s5ptv_status.vl_basic_param.win_blending 	= (vparam.flags & V4L2_FBUF_FLAG_CHROMAKEY) ? 1 : 0;
		s5ptv_status.vl_basic_param.alpha		= (vparam.flags & V4L2_FBUF_FLAG_LOCAL_ALPHA) ? 1 : 0;
		...
		if (s5ptv_status.vp_layer_enable) {
			...
			_s5p_vlayer_set_blending(s5ptv_status.vl_basic_param.win_blending);
...
/*
static int s5p_tv_v4l2_try_fmt_vid_out( struct file *file, void *fh, struct v4l2_format *f)
{
	return 0;
}
*/
...
/* Buffer handlers */
static int s5p_tv_v4l2_reqbufs(struct file *file, void *fh, struct v4l2_requestbuffers *b)
{
	return 0;
}

static int s5p_tv_v4l2_querybuf(struct file *file, void *fh, struct v4l2_buffer *b)
{
	return 0;
}

static int s5p_tv_v4l2_qbuf(struct file *file, void *fh, struct v4l2_buffer *b)
{
	return 0;
}

static int s5p_tv_v4l2_dqbuf(struct file *file, void *fh, struct v4l2_buffer *b)
{
	return 0;
}
...
static int s5p_tv_v4l2_g_fbuf(struct file *file, void *fh, struct v4l2_framebuffer *a)
{
...
static int s5p_tv_v4l2_s_fbuf(struct file *file, void *fh, struct v4l2_framebuffer *a)
{
	...
	case V4L2_PIX_FMT_RGB565 :
		s5ptv_overlay[layer->index].fb.fmt.pixelformat = VM_DIRECT_RGB565;
		break;

	case V4L2_PIX_FMT_RGB555 :
		s5ptv_overlay[layer->index].fb.fmt.pixelformat = VM_DIRECT_RGB1555;
		break;

	case V4L2_PIX_FMT_RGB444 :
		s5ptv_overlay[layer->index].fb.fmt.pixelformat = VM_DIRECT_RGB4444;
		break;

	case V4L2_PIX_FMT_RGB32 :
		s5ptv_overlay[layer->index].fb.fmt.pixelformat = VM_DIRECT_RGB8888;
		break;
...
/* Stream on/off */
static int s5p_tv_v4l2_streamon(struct file *file, void *fh, enum v4l2_buf_type i)
{
	s5p_tv_vo *layer = (s5p_tv_vo *)fh;
...
static int s5p_tv_v4l2_streamoff(struct file *file, void *fh, enum v4l2_buf_type i)
{
	s5p_tv_vo *layer = (s5p_tv_vo *)fh;
...
static int s5p_tv_v4l2_s_std(struct file *file, void *fh, v4l2_std_id *norm)
{
	...
	do {
		if (s5p_tv_standards[i].id == std_id) {
			s5ptv_status.v4l2.std = (struct v4l2_standard *) & s5p_tv_standards[i];
			break;
		}
	...
	if (i >= S5P_TVOUT_MAX_STANDARDS || s5ptv_status.v4l2.std == NULL) {
		V4L2PRINTK("(ERR) There is no tv-out standards : index = 0x%08Lx\n", std_id);
		return -EINVAL;
	}
...
static int s5p_tv_v4l2_querystd(struct file *file, void *fh, v4l2_std_id *a)
{
	return 0;
}

/* Output handling */
static int s5p_tv_v4l2_enum_output(struct file *file, void *fh, struct v4l2_output *a)
{
...
static int s5p_tv_v4l2_s_output(struct file *file, void *fh, unsigned int i)
{
	...
	case V4L2_OUTPUT_TYPE_YPBPR_INERLACED :
		s5ptv_status.tvout_param.out_mode = TVOUT_OUTPUT_COMPONENT_YPBPR_INERLACED;
		break;
...
/* Control handling */
static int s5p_tv_v4l2_queryctrl(struct file *file, void *fh, struct v4l2_queryctrl *a)
{
	return 0;
}

static int s5p_tv_v4l2_g_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	return 0;
}

static int s5p_tv_v4l2_s_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	return 0;
}

static int s5p_tv_v4l2_g_ext_ctrls(struct file *file, void *fh, struct v4l2_ext_controls *a)
{
	return 0;
}

static int s5p_tv_v4l2_s_ext_ctrls(struct file *file, void *fh, struct v4l2_ext_controls *a)
{
	return 0;
}

static int s5p_tv_v4l2_try_ext_ctrls(struct file *file, void *fh, struct v4l2_ext_controls *a)
{
	return 0;
}

static int s5p_tv_v4l2_querymenu(struct file *file, void *fh, struct v4l2_querymenu *a)
{
	return 0;
}

/* Audio ioctls */
static int s5p_tv_v4l2_enumaudio(struct file *file, void *fh, struct v4l2_audio *a)
{
	return 0;
}

static int s5p_tv_v4l2_g_audio(struct file *file, void *fh, struct v4l2_audio *a)
{
	return 0;
}

static int s5p_tv_v4l2_s_audio(struct file *file, void *fh, struct v4l2_audio *a)
{
	return 0;
}

/* Audio out ioctls */
static int s5p_tv_v4l2_enumaudout(struct file *file, void *fh, struct v4l2_audioout *a)
{
	return 0;
}

static int s5p_tv_v4l2_g_audout(struct file *file, void *fh, struct v4l2_audioout *a)
{
	return 0;
}

static int s5p_tv_v4l2_s_audout(struct file *file, void *fh, struct v4l2_audioout *a)
{
	return 0;
}

static int s5p_tv_v4l2_g_modulator(struct file *file, void *fh, struct v4l2_modulator *a)
{
	return 0;
}

static int s5p_tv_v4l2_s_modulator(struct file *file, void *fh, struct v4l2_modulator *a)
{
	return 0;
}
...
static int s5p_tv_v4l2_cropcap(struct file *file, void *fh, struct v4l2_cropcap *a)
{
	...
	V4L2PRINTK("[input type 0x%08x] : left: [%d], top [%d], width[%d], height[%d]\n",
...
static int s5p_tv_v4l2_s_crop(struct file *file, void *fh, struct v4l2_crop *a)
{
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT :
		...
		if (s5ptv_status.vp_layer_enable) {
			...
			_s5p_vlayer_set_dest_size((unsigned long)&img_size);
			_s5p_vlayer_set_dest_position((unsigned long)&img_offset);
		}
...
static int s5p_tv_v4l2_g_parm_v(struct file *file, void *fh, struct v4l2_streamparm *a)
{
	...
	memcpy(param->parm.raw_data, &vparam, sizeof(struct v4l2_window_s5p_tvout));

	return 0;
}

static int s5p_tv_v4l2_s_parm_v(struct file *file, void *fh, struct v4l2_streamparm *a)
{
...
static int s5p_tv_v4l2_g_parm_vo(struct file *file, void *fh, struct v4l2_streamparm *a)
{
	...
	vparam.win.chromakey	= s5ptv_overlay[layer->index].blank_color;			// clolr key value
	...
	vparam.win.global_alpha	= s5ptv_overlay[layer->index].win.global_alpha;		//value

	vparam.win.w.width	= s5ptv_overlay[layer->index].fb.fmt.bytesperline;
...
static int s5p_tv_v4l2_s_parm_vo(struct file *file, void *fh, struct v4l2_streamparm *a)
{
	...
	s5ptv_overlay[layer->index].blank_change = (vparam.flags & V4L2_FBUF_FLAG_CHROMAKEY) ? 1 : 0;
	s5ptv_overlay[layer->index].pixel_blending = (vparam.flags & V4L2_FBUF_FLAG_LOCAL_ALPHA) ? 1 : 0;
	s5ptv_overlay[layer->index].pre_mul	= (vparam.flags & V4L2_FBUF_FLAG_PRE_MULTIPLY) ? 1 : 0;
	s5ptv_overlay[layer->index].priority = vparam.priority;
	s5ptv_overlay[layer->index].blank_color = vparam.win.chromakey;			// clolr key value
	...
	s5ptv_overlay[layer->index].win.global_alpha = vparam.win.global_alpha;		//value
...
static int s5p_tv_v4l2_s_hw_freq_seek(struct file *file, void *fh, struct v4l2_hw_freq_seek *a)
{
	return 0;
}
...
//		*prot = hdcp_protocol_status;
	...
	case VIDIOC_ENUMSTD: {
		...
		memcpy(p, &s5p_tv_standards[p->index], sizeof(struct v4l2_standard));
...
long s5p_tv_vo_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VIDIOC_ENUM_FMT: {
		...
		if (type == V4L2_BUF_TYPE_VIDEO_OUTPUT_OVERLAY) {
			...
			return s5p_tv_v4l2_enum_fmt_vid_out_overlay(file, fh, f);
		}
...
const struct v4l2_ioctl_ops s5p_tv_v4l2_v_ops = {
	...
	.vidioc_g_priority			= s5p_tv_v4l2_g_priority,
	.vidioc_s_priority			= s5p_tv_v4l2_s_priority,
...
//	.vidioc_try_fmt_vid_out_overlay	= s5p_tv_v4l2_try_fmt_vid_out_overlay,			
	...
	.vidioc_querystd		= s5p_tv_v4l2_querystd,
	.vidioc_enum_output		= s5p_tv_v4l2_enum_output,
	.vidioc_g_output		= s5p_tv_v4l2_g_output,
	.vidioc_s_output		= s5p_tv_v4l2_s_output,
	.vidioc_queryctrl		= s5p_tv_v4l2_queryctrl,
	.vidioc_g_ctrl			= s5p_tv_v4l2_g_ctrl,
	.vidioc_s_ctrl			= s5p_tv_v4l2_s_ctrl,
	.vidioc_g_ext_ctrls		= s5p_tv_v4l2_g_ext_ctrls,
	.vidioc_s_ext_ctrls		= s5p_tv_v4l2_s_ext_ctrls,
	.vidioc_try_ext_ctrls		= s5p_tv_v4l2_try_ext_ctrls,
	.vidioc_querymenu		= s5p_tv_v4l2_querymenu,
	.vidioc_enumaudio		= s5p_tv_v4l2_enumaudio,
	.vidioc_g_audio			= s5p_tv_v4l2_g_audio,
	.vidioc_s_audio			= s5p_tv_v4l2_s_audio,
	.vidioc_enumaudout		= s5p_tv_v4l2_enumaudout,
	.vidioc_g_audout		= s5p_tv_v4l2_g_audout,
	.vidioc_s_audout		= s5p_tv_v4l2_s_audout,
	.vidioc_g_modulator		= s5p_tv_v4l2_g_modulator,
	.vidioc_s_modulator		= s5p_tv_v4l2_s_modulator,
	...
	.vidioc_log_status		= s5p_tv_v4l2_log_status,
...
const struct v4l2_ioctl_ops s5p_tv_v4l2_vo_ops = {
	...
	.vidioc_dqbuf			= s5p_tv_v4l2_dqbuf,
