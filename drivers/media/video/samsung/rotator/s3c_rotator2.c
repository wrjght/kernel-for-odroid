...
#include <asm/div64.h>
#include <plat/regs-rotator2.h>

#include "s3c_rotator_common2.h"
...
void s3c_rotator_set_source(ro_params *params)
{
	...
	__raw_writel(params->src_addr_cb, s3c_rotator_base + S3C_ROTATOR_SRCBASEADDR1);
	__raw_writel(params->src_addr_cr, s3c_rotator_base + S3C_ROTATOR_SRCBASEADDR2);
	/* src size */
...
int s3c_rotator_check_restrictions(ro_params *params)
{
	unsigned int align;
	...
	unsigned int check_align = 0, check_srcsize = 0, check_dstsize = 0;

	switch (params->src_format) {
	case ROTATOR_YCbCr420_3P:
		align = 16;
		minwidth = 64;
		minheight = 32;
		maxwidth = 64 * 1024;
	...
	check_align = (params->src_width % align) + (params->src_height % align) \
...
unsigned int s3c_rotator_get_status(void)
{
	...
	cfg = __raw_readl(s3c_rotator_base + S3C_ROTATOR_STATUS);
	cfg &= S3C_ROT_CONFIG_STATUS_MASK;
...
void s3c_rotator_enable_int(void)
{
	...
	cfg |= S3C_ROT_CONFIG_ENABLE_INT;

	__raw_writel(cfg, s3c_rotator_base + S3C_ROTATOR_CONFIG);
}
...
void s3c_rotator_set_inputfmt(rot_inputfmt inputfmt)
{
	unsigned int cfg;

	cfg = __raw_readl(s3c_rotator_base + S3C_ROTATOR_CTRL);
	cfg &= ~S3C_ROT_CTRL_INPUT_FMT_MASK;
...
irqreturn_t s3c_rotator_irq(int irq, void *dev_id)
{
	...
	cfg = __raw_readl(s3c_rotator_base + S3C_ROTATOR_STATUS);
	cfg |= S3C_ROT_STATREG_INT_PENDING;
