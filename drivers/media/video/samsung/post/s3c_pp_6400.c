...
static int s3c_pp_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case S3C_PP_SET_SRC_BUF_ADDR_PHY:
		...
		get_user(current_instance->src_buf_addr_phy_rgb_y, &parg->src_buf_addr_phy_rgb_y);
		get_user(current_instance->src_buf_addr_phy_cb, &parg->src_buf_addr_phy_cb);
		get_user(current_instance->src_buf_addr_phy_cr, &parg->src_buf_addr_phy_cr);
	...
	case S3C_PP_SET_SRC_BUF_NEXT_ADDR_PHY:
		...
		if (current_instance->instance_no != s3c_pp_instance_info.fifo_mode_instance_no) { 
			...
			get_user(current_instance->src_next_buf_addr_phy_rgb_y, &parg->src_next_buf_addr_phy_rgb_y);
			get_user(current_instance->src_next_buf_addr_phy_cb, &parg->src_next_buf_addr_phy_cb);
			get_user(current_instance->src_next_buf_addr_phy_cr, &parg->src_next_buf_addr_phy_cr);
	...
	case S3C_PP_SET_DST_BUF_ADDR_PHY:
		...
		get_user(current_instance->dst_buf_addr_phy_rgb_y, &parg->dst_buf_addr_phy_rgb_y);
		get_user(current_instance->dst_buf_addr_phy_cb, &parg->dst_buf_addr_phy_cb);
		get_user(current_instance->dst_buf_addr_phy_cr, &parg->dst_buf_addr_phy_cr);
