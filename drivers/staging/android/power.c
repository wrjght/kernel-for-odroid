...
static int android_power_debug_mask =
	ANDROID_POWER_DEBUG_USER_STATE | ANDROID_POWER_DEBUG_EXIT_SUSPEND | ANDROID_POWER_DEBUG_SUSPEND;
...
static void android_power_suspend(struct work_struct *work)
{
	...
	printk("android_power_suspend: Function start\n");
...
ssize_t acquire_partial_wake_lock_kernel(char *buf)
{
	int i;	
	size_t n = strlen(buf);
	unsigned long irqflags;
...
ssize_t release_wake_lock_kernel(char *buf)
{
	int i;
	size_t n = strlen(buf);
	unsigned long irqflags;
