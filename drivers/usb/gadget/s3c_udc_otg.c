...
#ifdef CONFIG_BATTERY_MAX17040	// odroid
...
//	s3c_cable_check_status(0);
	...
	#if defined(CONFIG_HAS_WAKELOCK)
		wake_unlock(&usb_wakelock);
	#endif
}
#endif /* CONFIG_MACH_HKDKC100 */
...
static int s3c_udc_probe(struct platform_device *pdev)
{
	...
	dev->is_pm_lock = false;
	#if defined(CONFIG_HAS_WAKELOCK)
		wake_lock_init(&usb_wakelock, WAKE_LOCK_SUSPEND, "usb_wakelock");
	#endif
#endif 
