...
#define DRIVER_NAME     "s5pc100-onenand"
...
struct clk *oneclk; /* to hold the pointer to onenand clock*/
...
//	printk("onenand_read_ops_nolock: from = 0x%08x, len = %i\n", (unsigned int) from, (int) len);
	...
	while (read < len) {
		...
		case ONENAND_READ_DMA:
			printk("OneNAND: ONENAND_READ_DMA\n");
...
static int __devinit s5pc100_onenand_probe(struct platform_device *dev)
{
	...
	printk(KERN_DEBUG "OneNAND: s5pc100_onenand_probe - (Virt) Base Address: 0x%08X\n",
	...
	/* enable the clock for onenand, this is only the workaround, need to shift in device specific code*/
        oneclk = clk_get(&pdev, "onenand");
        if (IS_ERR(oneclk)){
                panic("Failed to get OneNAND clock.\n");
        }
        clk_enable(oneclk);
...
#ifdef CONFIG_PM
#include <plat/pm.h>
static int s5pc100_onenand_suspend(struct platform_device *pdev, pm_message_t pm)
{
//	printk("OneNAND: s5pc100_onenand_suspend \n");
	return 0;
}

static int s5pc100_onenand_resume(struct platform_device *pdev)
{
	struct onenand_info *tmp = dev_get_drvdata(&pdev->dev);	
	struct onenand_chip *this = tmp->mtd.priv;
 //       printk("OneNAND: s5pc100_onenand_resume]\n");    // DEBUG

	/* Enable the OneNAND clock*/
        clk_enable(oneclk);
	/* Access Clock Register */
        writel(ONENAND_ACC_CLOCK_134_67, this->base + ONENAND_REG_ACC_CLOCK);
	...
	this->write(ONENAND_FSA_SHIFT, this->base + ONENAND_REG_OFFSET_ADDR);
        ...
        /* Unlock whole block */
        onenand_unlock_all(&tmp->mtd);

	return 0;
}
#else
#define s5pc100_onenand_suspend		NULL
#define s5pc100_onenand_resume		NULL
#endif	// end of #ifdef CONFIG_PM

static int __devexit s5pc100_onenand_remove(struct platform_device *dev)
{
	struct platform_device *pdev = dev;
...
static struct platform_driver s5pc100_onenand_driver = {
        .probe          = s5pc100_onenand_probe,
        .remove         = __devexit_p(s5pc100_onenand_remove),
        .suspend        = s5pc100_onenand_suspend,
        .resume         = s5pc100_onenand_resume,
        .shutdown       = NULL,         // TO DO
        .driver         = {
                .name   = DRIVER_NAME,
                .owner  = THIS_MODULE,
...
static int __init s5pc100_onenand_init(void)
{
        printk(KERN_INFO "S5PC100 OneNAND Driver.\n");

        return platform_driver_register(&s5pc100_onenand_driver);
}

static void __exit s5pc100_onenand_exit(void)
{
        platform_driver_unregister(&s5pc100_onenand_driver);
}

module_init(s5pc100_onenand_init);
module_exit(s5pc100_onenand_exit);
